...
- **Cooperation cost c.** The cost paid by a focal individual when it helps a
  ...
  hamilton is the equal-cost diagonal c0 = c1 = c. This is the *demand* for
...
- **pop_3** — one evolving population against a fixed partner (held at 25% of each
  genotype). Provably redundant with hamilton pop_3 (see synthesis.md and the
...
The single-population case (pop_1, and its diagonal generalisation hamilton) is the
...
In hamilton pop_2 (symmetric payoffs) the role split is stochastic; in mutualism
...
- **Behaviour vs machinery decoupling.** Under information cost, cooperation can stay
  ...
  by tax-free unconditional cooperators (C1P0, C1M0). See hamilton_cost.md.
