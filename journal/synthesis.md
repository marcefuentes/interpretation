...
symmetric_c and asymmetric_c0_c1 show *that* each mechanism collapses as the cost c rises,
...
This document maps that attribution back onto the specific symmetric_c and
asymmetric_c0_c1 thresholds, consolidating the cross-checks otherwise scattered across
...
| Mechanism family | Prisoners −b/a | Limiting axis              | symmetric_c / asymmetric_c0_c1 threshold it explains                    |
| ---------------- | -------------- | -------------------------- | ------------------------------------------------------------- |
| M                | 1.73           | risk / defection payoff P  | M's c-collapse is mostly rising risk, not a vanishing R − P    |
| P                | 0.89           | cooperation advantage R − P | the chooser bottleneck and the asymmetric_c0_c1 role split track R − P |
...
- **The symmetric_c c-collapse is largely a risk effect.** M noshuffle holds
  0.915 at c = 0.10 and 0.754 at c = 0.30, then collapses to 0.084 at c = 0.40.
  symmetric_c cannot tell whether the collapse is the shrinking R − P or the
...
- **Snowdrift confirms it from the other side.** In snowdrift S is high
  ...
  risk, not high reward, is what rescues M. The asymmetric_c0_c1 mirror is the
...
- **Groupsize-invariant in both.** Prisoners M keeps −b/a = 1.73 → 1.72 from
  gs = 128 to gs = 4; symmetric_c M noshuffle is flat across groupsize (0.906 vs
...
- **symmetric_c chooser bottleneck = R − P collapse.** symmetric_c P (shuffle,
...
- **asymmetric_c0_c1's deterministic role split is an R − P asymmetry.** Because
...
- **High-c persistence in symmetric_c.** At c = 0.40 (maximum risk and minimum
...
- **Pop_1 lift in asymmetric_c0_c1.** Combined mechanisms raise the exploited
...
- **Shuffle removes M-dependent cooperation.** symmetric_c M 0.915 → 0.053 at
  c = 0.10; asymmetric_c0_c1 MP reverts from 0.606 / 0.410 (Pop_0 / Pop_1) to the
...
- **gs = 4 removes P-dependent cooperation.** symmetric_c/asymmetric_c0_c1 P collapses in
  small groups, while MP / MPQ *recover* via their M component (e.g. symmetric_c
...
- **Symmetric payoffs split stochastically.** Prisoners pop_2 (symmetric, like
  ...
  symmetric_c pop_2.
- **Asymmetric payoffs split deterministically.** asymmetric_c0_c1 pop_2 has a
...
- **On the asymmetric branch there is no harmless Cost edge.** asymmetric_c1_i fixes
  c0 = 0.10 and sweeps Cost x c1, so even the cheap population always faces
  nonzero demand. The result is the complement of symmetric_c_i: Cost no longer
...
- **Structural, not literal.** Prisoners fixes S = 0.10, whereas symmetric_c's
  ...
  symmetric_c numbers sit on different scales.
...
| Phenomenon (symmetric_c / asymmetric_c0_c1)            | Prisoners attribution                      |
| -------------------------------------------- | ------------------------------------------ |
...
| Information cost Cost erodes cooperation slowly at c0 = c1 | machinery is shed harmlessly at c = 0; behavior decouples from mechanism |
