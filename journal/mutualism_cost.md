...
This is the missing asymmetric counterpart to symmetric_cost. On the diagonal
c0 = c1, symmetric_cost asks how the price of the machinery interacts with the
...
For the symmetric Cost x c sweep, see [symmetric_cost.md](symmetric_cost.md).
...
reference point: M, IMP, and IJMPQ are almost unchanged from symmetric_cost pop_2
...
The key difference from symmetric_cost is simple: there is no c = 0 refuge here.
...
to symmetric_cost's c = 0 edge, because there is no edge where enforcement can be
...
This is the asymmetric version of the price-demand interaction from symmetric_cost:
...
As in symmetric_cost, the control isolates the pure machinery tax. Mechanism M is
enough to show it:
...
symmetric_cost's striking result was that cooperation could remain high after the
...
population falls with it. This is the clean contrast with symmetric_cost: the
...
So the snowdrift lesson from symmetric_cost still holds only half-way. High S makes
...
symmetric_cost showed that information cost can be shed harmlessly when the game
