...
With matched costs (c₀ = c₁), partner choice produces a stochastic cooperator/exploiter assignment in the prisoner's dilemma that is absent without enforcement (a–d); the cooperating side can earn lower fitness (paradox of success). In snowdrift the same split already appears in the no-enforcement columns; partner choice mainly reshapes high-cost outcomes (e–h). Fig. 3 converts the chance split into a deterministic one; Fig. S5 contrasts the two strips on shared axes.
...
Along c₁ = c₀ + 0.02 with both populations overlaid, partner choice converts the R − P gap into a stable cooperator/exploiter assignment (a, b); combined IJMPQ lifts the expensive population and shrinks the outcome asymmetry (c, d). Prisoner's dilemma only: under no enforcement the cheap population barely cooperates (Fig. S1), so partner choice creates the deterministic split from the parameter gap. In snowdrift the sucker payoff already sustains high cooperation without machinery (Fig. 2e–h), so a parallel asymmetric panel would repeat that floor. Full c₀ × c₁ coverage is in Fig. S9; the split survives group size 4 (Fig. S3).
...
Each panel holds one population's information cost at zero and sweeps the other's, separating the cost a population pays from the cost its partner pays. Under partner choice (a, b) both populations are nearly flat in their own cost and collapse under the partner's — assortment needs an active chooser on each side and is limited by whichever supplies fewer. Under the combined reputation-rich mechanism (c, d) the binding axis swaps: taxing the low-cooperation-cost population to i = 0.20 drives its partner from 0.957 to 0.268 while the payer falls only to 0.734. Common x-range is the tighter per-population cap b − cₚ.
...
Total information cost is held at 0.2 while the split between the two populations varies along the x-axis, so every point costs the pair the same. Under the combined reputation-rich mechanism (a) both populations reach their lowest cooperation at an interior split rather than at either end: each extreme leaves one population with an intact apparatus, enough to hold the pair together, whereas a split breaks both. Partner choice (b) is monotone instead — only the high-cooperation-cost population's information cost bites — so the non-convexity is a property of reciprocity-bearing mechanisms, not of the budget.
...
Each panel fixes i₀ on the low-cooperation-cost population and sweeps i₁. The inversion — the expensive population's curve above the cheap one's — holds throughout when i₀ = 0 (a), survives only past a threshold when i₀ = 0.02 (b), and is gone by i₀ = 0.04 and 0.1 (c, d). Shaded ±1 SD bands over 30 runs mark a bistable threshold: the band is wide below it (runs split between two attractors) and collapses above it. Snowdrift removes the wedge entirely. The equal-c hitchhiking contrast is Fig. S6.

## Supplement figures

### <strong>Fig. S1.</strong> No-enforcement control for Fig. 3 (parameter-asymmetric two populations).
...
Shuffle-enabled short-memory variants of the mechanisms in Fig. 1. The direct-reciprocity collapse ordering can shift relative to the baseline hierarchy while the partner-choice versus combined contrast remains.

### <strong>Fig. S3.</strong> Cooperation-cost asymmetry under partner choice at group size 4.

![figS3](/home/marcelino/figures/interpretation/figS3.png)

<strong>Fig. S3.</strong> Short-memory partner choice. Frequency of cooperators and average fitness when short-memory partner choice is the only cooperation mechanism allowed. (<strong>a–d</strong>) Two coevolving populations. (<strong>a, b</strong>) Population with the higher cooperation cost (<em>c<sub>1</sub></em>, from 0.02 to <em>b</em>, the benefit to the partner). (<strong>c, d</strong>) Population with the lower cooperation cost (<em>c<sub>0</sub></em>, from 0 to <em>b</em> - 0.02). The two populations have the same information cost (0.001).

Same cooperation-cost parameter asymmetry as Fig. 3 but at group size 4. The deterministic cooperator/exploiter split survives small-group stochasticity.

### <strong>Fig. S4.</strong> Dilemma-0 control for machinery erosion.
...
Machinery alleles erode with (c, d) or without (a, b) a social dilemma; cooperation persists through the shed only when the dilemma is present. The information-cost tax drives the erosion; the dilemma decides whether losing the machinery drags behaviour down (cf. Fig. 4).
...
Top row: small cooperation-cost gap (deterministic split; same strip as Fig. 3). Bottom row: parameter symmetry (stochastic split; same data as Fig. 2). Contrast Fig. 2 control columns for the no-enforcement symmetric baseline.

### <strong>Fig. S6.</strong> Information-cost asymmetry at equal cooperation cost (c = 0.10).
...
Information cost is soft by itself but lowers the cooperation-cost ceiling where the two costs overlap. Full-coverage Cost × c grid behind Fig. 4, which shows the c = 0 machinery-versus-behaviour slice rather than combined IJMPQ alone.
...
Full c₀ × c₁ coverage behind Fig. 3; the main-text line slice follows c₁ = c₀ + 0.02 with both populations overlaid. Partner choice (a–d) and combined IJMPQ (e–h).
