...
<strong>Fig. 1.</strong> Simulation results for different cooperation mechanisms. Frequency of cooperators. The figure shows one population. (<strong>a, e</strong>) When no cooperation mechanisms are allowed. (<strong>b, f</strong>) When short-memory partner choice is the only cooperation mechanism allowed. (<strong>c, g</strong>) When direct reciprocity is the only cooperation mechanism allowed. (<strong>d, h</strong>) When partner choice and indirect reciprocity are both allowed. (<strong>a–d</strong>) Prisoner's dilemma. (<strong>e–h</strong>) Snowdrift. The population has the same cooperation cost (<em>c</em>, ranging from 0 to <em>b</em>, the benefit to the partner) and the same information cost (0.001).

Establishes the cooperation-cost threshold ordering for partner choice, direct reciprocity, and the combined reputation-rich mechanism in one coevolving population, with the leftmost column showing the no-enforcement control. Uses the same negligible information cost (0.001) as the two-population and information-cost figures below; only the mechanism column varies.
...
In the prisoner's dilemma, partner choice stochastically assigns cooperator and exploiter roles that are absent in the left control columns; the cooperator population can earn lower fitness (paradox of success). In snowdrift, a similar split already appears without enforcement machinery; partner choice mainly reshapes outcomes at high cooperation cost. Compare Fig. 3 and the line-slice contrast in Fig. S5.
...
Top row: small cooperation-cost gap (deterministic split; cf. Fig. 3). Bottom row: parameter symmetry (stochastic split; same data as Fig. 2). Contrast Fig. 2 control columns for the no-enforcement symmetric baseline.
