# Prisoner's Dilemma — Partner Choice

Partner-choice analysis for the prisoners calibration study (mechanism **P**).
Reciprocity mechanisms are documented in **prisoners_reciprocity.md**.

## Overview

This document summarizes insights from analyzing the heatmaps generated by
graphgen --study prisoners --figure s07.

Role in this project: **prisoners is a calibration study**, not a primary
endpoint. Its value is to clarify mechanism behavior (partner-choice bottlenecks,
bistability, hitchhiking, exploitation signatures) that helps interpret
**mutualism** as the main biological target and **Hamilton** as the
equal-parameter special case of mutualism.

It covers pop_3 (panels e-f, evolving population), pop_2 (panels a-d, two
coevolving populations), pop_1 (panel i, single population), the emergent
asymmetry and sharp phase transition in cooperation near the R ≈ P diagonal,
C1P0 free-riders at high R−P, and cross-study comparisons with the Hamilton
game.

**Partner choice mechanics** (mutual **C1P1** swaps, **C0P1** silent in
defectors) are given once in **The Resolution**; later sections add **grid and
single-run numbers** without re-deriving the code path every time.

**Groupsize / path.** Heatmaps and aggregated **prisoners/** statistics refer
to **shuffle_cost12_128** (groupsize **128**). **shuffle_cost12_4** yields
**much lower** **qBSeen** on the same *(R,P)* grid; where
**csv_0_for_movie.con** exists for **4**, trajectories are **plateaued by the
last snapshots** on the usual nine-checkpoint horizon—not **128** “still catching
up.” The detailed **hamilton_1run** movie comparison is in **hamilton.md**.
