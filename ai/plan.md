...
14. [ ] Asymmetric information-cost study (per-population Cost0, Cost1).
    ...
    - **Study A (symmetric c, asymmetric Cost) — the clean first study.**
      - pop_2 only. Fix c0 = c1 = c at a positive value so demand bites (c = 0 is a
        near-null: symmetric_c_i pop_2 barely breaks symmetry with no temptation).
        **c = 0.10 primary** (nests the Cost0 = Cost1 edge onto symmetric_c_i pop_2 at
        c = 0.10, and sits adjacent to asymmetric_c1_i's low-asymmetry edge) plus
      ...
      - Sweep (Cost0, Cost1) on a **0.02 grid** (matches symmetric_c_i / asymmetric_c1_i
      ...
      - **Ordering triangle Cost0 <= Cost1** (not a full square): under symmetric c the
        ...
        pure-Cost axis already covered by symmetric_c_i pop_2.
      - **Per-axis cap Cmax = b - c** (LOCKED), i.e. each axis bounded independently by
        ...
        as in symmetric_c_i, cells past that are kept but read knowing the combined
    ...
    - **No c = 0 slice** (would land in symmetric_c_i's harmless-shedding regime where
