...
Unlike hamilton_cost, this sweep fixes c0 = 0.10 for the low-cost population and
...
def paired_deltas(mech, d=1, sh="noshuffle", gs="128"):
    r0 = load(mc_path("mutualism_cost", sh, gs, mech, d, 0))
    r1 = load(mc_path("mutualism_cost", sh, gs, mech, d, 1))
...
for mech in MECHS:
    r0 = load(mc_path("mutualism_cost", "noshuffle", "128", mech, 1, 0))
    r1 = load(mc_path("mutualism_cost", "noshuffle", "128", mech, 1, 1))
...
for mech in PRIMARY_MECHS + ["MP", "MPQ"]:
    for f in (0, 1):
        r = load(mc_path("mutualism_cost", "noshuffle", "128", mech, 1, f))
        vals = []
        for co in SAMPLE_COSTS:
...
for co in SAMPLE_COSTS:
    ...
    for d in (0, 1, 2):
        for f in (0, 1):
            rows = load(mc_path("mutualism_cost", "noshuffle", "128", "M", d, f))
...
for mech, tok in (("P", "P1"), ("M", "M1")):
    print(f"  {mech}:")
    for f in (0, 1):
        rows = load(mc_path("mutualism_cost", "noshuffle", "128", mech, 1, f))
        print(f"    pop_{f}:")
...
for mech in ("P", "MP", "IMP", "IJMPQ"):
    ...
    for co in (0.0, 0.08, 0.12):
        ns0 = mean_at_cost(load(mc_path("mutualism_cost", "noshuffle", "128", mech, 1, 0)), co)
        ns1 = mean_at_cost(load(mc_path("mutualism_cost", "noshuffle", "128", mech, 1, 1)), co)
        sh0 = mean_at_cost(load(mc_path("mutualism_cost", "shuffle", "128", mech, 1, 0)), co)
        sh1 = mean_at_cost(load(mc_path("mutualism_cost", "shuffle", "128", mech, 1, 1)), co)
...
for mech in ("M", "P", "IJMPQ"):
    ...
    for co in (0.0, 0.08, 0.20):
        a0 = mean_at_cost(load(mc_path("mutualism_cost", "noshuffle", "128", mech, 1, 0)), co)
        a1 = mean_at_cost(load(mc_path("mutualism_cost", "noshuffle", "128", mech, 1, 1)), co)
        b0 = mean_at_cost(load(mc_path("mutualism_cost", "noshuffle", "4", mech, 1, 0)), co)
        b1 = mean_at_cost(load(mc_path("mutualism_cost", "noshuffle", "4", mech, 1, 1)), co)
...
for mech in ("M", "P", "IJMPQ"):
    ...
    for d in (1, 2):
        ...
        for co in (0.0, 0.08, 0.20, 0.28):
            r0 = mean_at_cost(load(mc_path("mutualism_cost", "noshuffle", "128", mech, d, 0)), co)
            r1 = mean_at_cost(load(mc_path("mutualism_cost", "noshuffle", "128", mech, d, 1)), co)
            line += f"  C{co:.2f}:{r0:.3f}/{r1:.3f}"
        print(line)

print("\n--- I. TEMPORAL SAMPLES (mutualism_cost_1run movies, ns gs128 PD) ---")
for mech, co, c1 in (("P", 0.00, 0.20), ("P", 0.12, 0.20),
                     ("M", 0.00, 0.20), ("IJMPQ", 0.12, 0.20),
                     ("IJMPQ", 0.20, 0.20)):
    print(f"  {mech} at (Cost={co:.2f}, c1={c1:.2f})")
    for f in (0, 1):
        rows = load(mc_path("mutualism_cost_1run", "noshuffle", "128", mech, 1, f, movie=True))
...
print("\n--- J. SYMMETRIC REFERENCE: hamilton_cost pop_2 at c=0.10 vs mutualism_cost c1=0.12 ---")
for mech in PRIMARY_MECHS:
    ...
    m0 = mc_cell(load(mc_path("mutualism_cost", "noshuffle", "128", mech, 1, 0)), 0.0, 0.12)
    m1 = mc_cell(load(mc_path("mutualism_cost", "noshuffle", "128", mech, 1, 1)), 0.0, 0.12)
    print(f"  {mech}: hamilton {h0:.3f}/{h1:.3f}  mutualism_cost {m0:.3f}/{m1:.3f}")
