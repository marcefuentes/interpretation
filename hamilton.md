...
This document summarizes insights from analyzing the line plots generated by graphgen --study hamilton --figure s07 (partner choice under the **P** mechanism only: **no** direct or indirect reciprocity). It covers pop_1 (panels e-f, single population), pop_2 (panels a-b, two coevolving populations), and pop_3 (panels c-d, one population adapting to a fixed population). The Hamilton study uses a continuous altruism game parameterized by benefit *b* and cost *c*, with the *b* − *c* axis as the control parameter. Hamilton is the diagonal special case of mutualism (*b*0 = *b*1, same *c*). To avoid repeating shared model mechanics, use **mutualism.md** for the general cross-study framing (payoff structure, bottleneck logic, diagonal relation); this file deals with Hamilton-specific patterns and numbers.
